...
//   it('Test Function Directory with Valid Credentials', () => {
//     cy.visit('https://opensource-demo.orangehrmlive.com/web/index.php/auth/login');
//     cy.intercept("GET","**/employees/action-summary").as("actionsummary");
//     loginPage.inputUsername().type('Admin');
//     loginPage.inputPassword().type('admin123');
...
//     cy.wait(4000);
//     loginPage.inputHints().type('Peter Mac Anderson');
//     cy.get('.oxd-select-text--active').eq(0).click();
...
describe('Fitur Directory', () => {
  it('Test Function Directory with Valid Credentials', () => {
    ...
    loginPage.buttonDirectory().click();
    cy.wait(4000);
    loginPage.inputHints().type('Rebecca Harmony');
    cy.get('.oxd-select-text--active').eq(0).click();
